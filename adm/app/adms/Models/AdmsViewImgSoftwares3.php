 ...
 * Class
 */
class AdmsViewImgSoftwares3 extends Conn
{
	private object $conn;
	private $dados;
	...
	public function editImgSoft($dados){
		$this->dados = $dados;
		$this->conn = $this->connect();
		$query_soft = "UPDATE homes_sotwares SET icone_soft_3=:icone_soft_3, modified=NOW() WHERE id=:id";
		$edit_soft = $this->conn->prepare($query_soft);
		$edit_soft->bindParam(':icone_soft_3', $this->dados['img_nova']['name']);
		$edit_soft->bindParam(':id', $this->dados['id'], PDO::PARAM_INT);
		$edit_soft->execute();

		if ($edit_soft->rowCount()) {
			if ($this->upload()) {
				$_SESSION['msg_soft'] = '<div class="alert alert-success" role="alert">A imagem da Ferramenta 3 foi editado!</div>';
				return true;
			}else{
				$_SESSION['msg_soft'] = '<div class="alert alert-danger" role="alert">A imagem da Ferramenta 3 NÃO foi editado!!</div>';
				return false;
			}		
	...
	private function apagarImagem(){
		$img_antiga = "app/adms/assets/images/icons_tools/" . $this->dados['icone_soft_3'];
		unlink($img_antiga);
	}
